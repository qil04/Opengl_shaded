...
CCylinder::CCylinder(void) {
    pCylinder = new Cylinder(2, 2, 10, 108, 8);
}
...
void CCylinder::InitGL()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, pCylinder->getInterleavedVertexSize(), pCylinder->getInterleavedVertices(), GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, pCylinder->getIndexSize(), pCylinder->getIndices(), GL_STATIC_DRAW);

    int stride = pCylinder->getInterleavedStride();
...
void CCylinder::RenderGL()
{
    glUseProgram(m_program);
    glBindVertexArray(m_vao);
    glDrawElements(GL_TRIANGLES, pCylinder->getIndexCount(), GL_UNSIGNED_INT, (void*)0);
}